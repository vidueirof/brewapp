...
if [[ "$TRAVIS_BRANCH" == "$HOCKEYAPP_UPLOAD_BRANCH" ]]; then  
  ...
  curl https://rink.hockeyapp.net/api/2/apps/$HOCKEY_APP_ID/app_versions/upload -F status="2" -F notify="0" -F notes="$DELIVER_WHAT_TO_TEST" -F ipa="@$OUTPUTDIR/$APP_NAME.ipa" -F dsym="@$OUTPUTDIR/$APP_NAME.dsym.zip" -F commit_sha="$TRAVIS_COMMIT" -H "X-HockeyAppToken: $HOCKEY_APP_TOKEN"
